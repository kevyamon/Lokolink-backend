...
const createSession = async (req, res) => {
  ...
  try {
    ...
    // Note: On accepte une liste vide si le délégué compte sur les invitations
    // if (sponsorsArray.length === 0) ... (Optionnel)
...
const getMySession = async (req, res) => {
  try {
    ...
    // Vérifier que c'est bien le créateur qui demande
    // (Ou un admin supreme)
...
const joinSession = async (req, res) => {
  ...
  try {
    ...
    // SOCKET : IMPORTANT - On envoie la session mise à jour pour le dashboard !
    req.io.emit('session:updated', session);
...
module.exports = {
  createSession,
  getMySession, // <--- NOUVEAU
  deleteSession,
  getActiveSessions,
  getSessionDetails,
  joinSession, // <--- NOUVEAU (de l'étape précédente)
};